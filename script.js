...
function render() {
    ...
    switch (state.activeView) {
        ...
        case 'studentDetail': viewContent = renderStudentDetailView(); break;
...
function renderActivityDetailView() {
    ...
    // --- Lógica de Navegación de Sesiones ---
...
function handleAction(action, element, event) {
    ...
    // Acciones que necesitan un re-renderizado completo
...
const actionHandlers = {
    ...
    'change-activity-color': (id, element) => {
         ...
         if(activity) {
            ...
            saveState();
            if(state.activeView === 'schedule' || state.activeView === 'classes' || state.activeView === 'settings') {
                render();
            }
         }
    },
...
function findSession(activityId, fromDate, direction) {
    ...
    // Start from the next/previous day to avoid finding the same day
    currentDate.setDate(currentDate.getDate() + increment);

    // Search for a maximum of 1 year to avoid infinite loops
    for (let i = 0; i < 365; i++) {
        const dayOfWeek = (currentDate.getDay() + 6) % 7; // Monday = 0
        
        if (dayOfWeek < 5) { // Only check Monday to Friday
            ...
            for (const time of slotsToCheck) {
                if (state.schedule[`${dayName}-${time.label}`] === activityId) {
                    // Check if this found session is within the valid date ranges
                    const courseStartDate = state.courseStartDate ? new Date(state.courseStartDate + 'T00:00:00') : null;
                    const courseEndDate = state.courseEndDate ? new Date(state.courseEndDate + 'T23:59:59') : null;
...
function findNextSession(activityId, fromDate) {
    return findSession(activityId, fromDate, 'next');
}

function findPreviousSession(activityId, fromDate) {
